...
export interface DiepBarrelDefinition {
    angle: number;
    offset: number;
    distance?: number;
    size: number;
...
export interface DiepTankDefinition {
    id: number
    name: string;
    ...
    barrels: DiepBarrelDefinition[];
    //stats: StatDefinition[];
    [key:string]: any;
}

const diepTankDefinitions: Array<DiepTankDefinition|null> = [
    {
        "id": 0,
        "name": "Tank",
        "upgradeMessage": "",
...
export function getTankByID(id: string): DiepTankDefinition {
    return (diepTankDefinitions[0] || diepTankDefinitions.find(tank => tank != null && tank.name.replaceAll(" ", "-").toLowerCase() == id)) as DiepTankDefinition
}